...
export function releaseForOrigin(release: ReleaseDescriptor, origin?: string): ReleaseDescriptor {
  if (!origin || !origins.has(origin)) return release;
  const url = new URL(release.url);
  if (!origins.has(url.origin) || !url.pathname.startsWith('/download/android/files/')) return release;
