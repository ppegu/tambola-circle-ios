...
export function listingMembers(state: RoomState) {
  ...
  return {
    tableAvatarId: state.tableAvatarId ?? 0,
    tableAvatarPhoto: state.tableAvatarPhoto,
    playerCount: members.length,
    players: members
      .slice(0, 4)
      .map(({ id, name, avatarId, avatarPhoto }) => ({
        id,
        name,
        avatarPhoto,
...
export function pastGame(state: RoomState): PastGame {
  return {
    ...
    updatedAt: state.result?.at ?? state.createdAt,
    endedAt: state.result?.at ?? state.createdAt,
    completed: !!state.result?.winner,
    winnerName: state.result?.winner
      ? (state.members[state.result.winner]?.name ?? null)
      : null,
...
export function pastGameDetails(state: RoomState): PastGameDetails {
  return {
    game: pastGame(state),
    calls: state.calls,
    reason: state.result?.reason ?? "Round ended",
...
export async function listTables(
  ...
  const rows = await db
    .prepare(
      `SELECT t.id,t.name,t.owner_id AS ownerId,t.round,t.phase,t.updated_at AS updatedAt,t.listing
    FROM online_tables t JOIN online_memberships m ON m.table_id=t.id
    WHERE m.player_id=? AND m.removed=0 ORDER BY t.updated_at DESC LIMIT 100`,
    )
    .bind(playerId)
    .all<TableSummary & { listing: string }>();
  return rows.results.map(({ listing, ...table }) => ({
    ...table,
    ...JSON.parse(listing),
  }));
}
...
export async function listPastGames(
  ...
  const rows = await db
    .prepare(
      `SELECT r.state ${archiveAccess}
    ORDER BY json_extract(r.state,'$.result.at') DESC,r.table_id,r.round DESC LIMIT 100`,
    )
    .bind(playerId)
    .all<{ state: string }>();
...
export async function getPastGame(
  db: D1Database,
  playerId: string,
  tableId: string,
  round: number,
): Promise<PastGameDetails | null> {
  const row = await db
    .prepare(`SELECT r.state ${archiveAccess} AND r.table_id=? AND r.round=?`)
    .bind(playerId, tableId, round)
    .first<{ state: string }>();
