...
export function generateStrip(kind: StripKind, random: RandomInt = secureInt): Panel[] {
  let counts: number[][] = [];
  for (let attempt = 0; attempt < 1000; attempt++) {
    ...
    for (const col of shuffled([0, 1, 2, 3, 4, 5, 6, 7, 8], random)) {
      ...
      for (let n = 0; n < extra; n++) {
        const candidates = remaining.map((v, i) => i).filter(i => remaining[i]! > 0 && counts[i]![col]! < 3);
        if (!candidates.length) { valid = false; break; }
        const index = candidates[random(candidates.length)]!;
        counts[index]![col]!++; remaining[index]!--;
      }
      if (!valid) break;
    }
  ...
  const panels: Panel[] = counts.map(count => {
    const panel: Panel = Array.from({ length: 3 }, () => Array<number | null>(9).fill(null));
    const capacity = [5, 5, 5];
    const cols = shuffled([0, 1, 2, 3, 4, 5, 6, 7, 8], random).sort((a, b) => count[b]! - count[a]!);
    const fill = (index: number): boolean => {
      if (index === 9) return capacity.every(n => n === 0);
      const col = cols[index]!, size = count[col]!;
      const options = size === 3 ? [[0, 1, 2]] : size === 2 ? [[0, 1], [0, 2], [1, 2]] : [[0], [1], [2]];
      for (const rows of shuffled(options, random)) {
        if (rows.some(row => capacity[row] === 0)) continue;
        for (const row of rows) { capacity[row]!--; panel[row]![col] = 0; }
        if (fill(index + 1)) return true;
        for (const row of rows) { capacity[row]!++; panel[row]![col] = null; }
      }
      return false;
    };
  ...
  for (let col = 0; col < 9; col++) {
    const low = col === 0 ? 1 : col * 10, length = col === 0 ? 9 : col === 8 ? 11 : 10;
    const pool = shuffled(Array.from({ length }, (_, i) => low + i), random);
    for (const panel of panels) {
      const rows = [0, 1, 2].filter(row => panel[row]![col] !== null);
      const numbers = pool.splice(0, rows.length).sort((a, b) => a - b);
      rows.forEach((row, index) => { panel[row]![col] = numbers[index]!; });
    }
  }
