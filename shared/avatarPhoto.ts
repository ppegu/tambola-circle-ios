...
export function avatarBytes(base64: string): Uint8Array {
  if (!base64.length || base64.length % 4 || base64.length > Math.ceil(MAX_AVATAR_BYTES / 3) * 4 || !/^[A-Za-z0-9+/]+={0,2}$/.test(base64)) throw new Error('Invalid photo data.');
  ...
  for (let i = 0, out = 0; i < base64.length; i += 4) {
    const bits = alphabet.indexOf(base64[i]!) << 18 | alphabet.indexOf(base64[i + 1]!) << 12 | Math.max(0, alphabet.indexOf(base64[i + 2]!)) << 6 | Math.max(0, alphabet.indexOf(base64[i + 3]!));
