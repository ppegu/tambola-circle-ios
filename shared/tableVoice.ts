...
export function isVoiceRequest(
  value: Record<string, unknown>,
): value is Record<string, unknown> & VoiceRequest {
  return (
    [
      "join",
      "status",
    ...
    /^[a-f0-9-]{36}$/.test(value.clientId)
  );
}

/** Reject video/data channels and bound SDP before it reaches the provider. */
export function validVoiceDescription(
  value: unknown,
  type: "offer" | "answer",
): value is VoiceDescription {
  ...
  return (
    media.length > 0 &&
    media.length <= VOICE_MAX_MEMBERS &&
    media.every((line) => line.startsWith("m=audio ")) &&
