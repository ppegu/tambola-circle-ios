...
describe("Tambola Circle public links", () => {
  it("uses the current name for existing and new tables even with older build configuration", () => {
    for (const origin of [
      ...
      expect(url).toContain("tambola-circle");
  ...
  it("normalizes old download share links and preserves custom/local endpoints", () => {
    expect(
      publicAppUrl(LEGACY_APP_ORIGIN + "/download/android?source=share"),
    ).toBe(PUBLIC_APP_ORIGIN + "/download/android?source=share");
    for (const value of [
      "",
      "http://127.0.0.1:8791",
      "https://private.example",
      LEGACY_APP_ORIGIN + ".example",
    ])
      expect(publicAppUrl(value)).toBe(value);
  });
  it("redirects old invite clicks while preserving the exact table and invite token", async () => {
    const fetch = vi.fn();
    for (const method of ["GET", "HEAD"]) {
      const response = await proxy.fetch(
        new Request(LEGACY_APP_ORIGIN + path, { method }),
        { TARGET: { fetch } as unknown as Fetcher },
      );
  ...
  it("still forwards API, signed APK downloads, voice packs and malformed invites unchanged", async () => {
    ...
    for (const path of [
      "/v2/devices/open",
      "/download/android/files/com.ppegu.tambola/11/file.apk",
      "/packs/latest.json",
      "/invite/invalid",
    ]) {
      const request = new Request(LEGACY_APP_ORIGIN + path);
      expect(
        await proxy.fetch(request, { TARGET: { fetch } as unknown as Fetcher }),
      ).toBe(response);
