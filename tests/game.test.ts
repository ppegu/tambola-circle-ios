...
describe("number caller", () => {
  it("draws all ninety numbers exactly once and then finishes", () => {
    ...
    for (let i = 0; i < 90; i++)
      history.push(drawNumber(history, (max) => (i * 13) % max)!);
  ...
  it("rejects corrupted saved games", () => {
    for (const value of [[1, 1], [0], [91], [1.5], ["1"], null, {}])
      expect(isHistory(value)).toBe(false);
  ...
  it("strictly validates cloud preferences", () => {
    ...
    expect(isPreferences({ ...DEFAULT_PREFERENCES, voice: "female" })).toBe(
      true,
    );
    expect(isPreferences({ ...DEFAULT_PREFERENCES, voice: "unknown" })).toBe(
      false,
    );
    expect(DEFAULT_PREFERENCES.callPause).toBe(1);
    for (const callPause of [0.5, 1, 1.5, 2])
      expect(isPreferences({ ...DEFAULT_PREFERENCES, callPause })).toBe(true);
    for (const callPause of [-1, 0, 0.75, 3, "1", null])
      expect(isPreferences({ ...DEFAULT_PREFERENCES, callPause })).toBe(false);
    for (const value of [
      { auto: true, speed: 0, sound: true },
      { ...DEFAULT_PREFERENCES, admin: true },
      { ...DEFAULT_PREFERENCES, sound: "yes" },
      null,
    ])
