...
vi.mock("react-native", () => ({
  Image: { resolveAssetSource: native.resolveAssetSource },
}));
import { getRandomBytes, randomUUID } from "../src/native/crypto";
import { createClipPlayer, voiceAssetUri } from "../src/playClip";
beforeEach(() => {
  vi.resetAllMocks();
  native.resolveAssetSource.mockReturnValue({
    uri: "assets_voice_en_female_1",
  });
  native.setAudioActive.mockResolvedValue(undefined);
});
describe("native cryptographic adapter", () => {
  ...
  it("fails closed when the native random source is malformed", () => {
    for (const value of ["", "zz", "fff"]) {
      native.randomHex.mockReturnValue(value);
...
describe("native clip lifetime", () => {
  ...
  it("ignores late completion after cancellation while a newer clip finishes", async () => {
    ...
    expect(failed).not.toHaveBeenCalled();
    expect(native.playClip.mock.calls[0]![0]).not.toBe(
      native.playClip.mock.calls[1]![0],
    );
    expect(native.stopClip).toHaveBeenCalledWith(
      native.playClip.mock.calls[0]![0],
    );
  });
  it("reports playback failure, but suppresses a cancellation rejection", async () => {
    ...
    const ended = vi.fn(),
      failed = vi.fn();
    createClipPlayer()(1, ended, failed);
    for (let i = 0; i < 8; i++) await Promise.resolve();
    expect(failed).toHaveBeenCalledTimes(1);
    expect(native.playClip).toHaveBeenCalledTimes(2);
    const cancel = createClipPlayer()(1, ended, failed);
    cancel();
    await Promise.resolve();
    expect(failed).toHaveBeenCalledTimes(1);
    expect(ended).not.toHaveBeenCalled();
  ...
  it("recreates a failed player once and does not report a recovered call as failed", async () => {
    native.playClip
      .mockRejectedValueOnce(new Error("player expired"))
    ...
    for (let i = 0; i < 8; i++) await Promise.resolve();
    expect(native.playClip).toHaveBeenCalledTimes(2);
    expect(native.setAudioActive).toHaveBeenCalledWith(true);
    expect(ended).toHaveBeenCalledOnce();
    expect(failed).not.toHaveBeenCalled();
  });
  it("rejects missing bundled assets before playback", () => {
    native.resolveAssetSource.mockReturnValue(null);
    expect(() => createClipPlayer()(1, vi.fn(), vi.fn())).toThrow(
