...
describe('game audio session ownership', () => {
  it('serializes leaving/re-entering a game', async () => {
    ...
    await Promise.all([setGameAudioActive(true), setGameAudioActive(false), setGameAudioActive(true)]);
    expect(audio.setAudioActive.mock.calls.map(call => call[0])).toEqual([true, false, true]);
