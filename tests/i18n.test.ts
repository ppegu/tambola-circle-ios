...
describe("bundled translations", () => {
  ...
  it("keeps user content and game numbers intact, without recursively translating values", () => {
    ...
    expect(translate("hi", "Hi, {name}!", { name })).toBe(
      "नमस्ते, " + name + "!",
    );
    expect(translate("as", "Ticket {v0} of {v1}", { v0: 2, v1: 6 })).toBe(
      "6খনৰ টিকট 2",
    );
    expect(translate("en", "{v0} coins", { v0: 0 })).toBe("0 coins");
    expect(translateKnown("hi", "Unknown server detail")).toBe(
...
describe("language selection persistence", () => {
  ...
  it("defaults to English but leaves first-run selection pending", async () => {
    ...
    expect(i18n.languageStore.getState()).toEqual({
      language: "en",
      selected: false,
      hydrated: true,
    });
  });
  it.each(["en", "as", "hi"] as const)(
    "restores a saved %s selection without prompting again",
    async (language) => {
  ...
  it.each(["invalid", null])(
    "keeps the choice open for unsupported or missing storage: %s",
    async (saved) => {
  ...
  it("does not let slow hydration overwrite a newer selection", async () => {
    ...
    const i18n = await import("../src/i18n");
    const loading = i18n.initializeLanguage();
    await i18n.selectLanguage("hi");
    finish("as");
    await loading;
