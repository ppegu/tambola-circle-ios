...
describe("complete caller pack release inputs", () => {
  it("includes ninety verified offline files per voice and only bundles Aria", () => {
    expect(catalog.packs.map((p) => p.id)).toEqual([
      "aria",
      "neerja",
      "ava",
      "emma",
    ...
    for (const pack of catalog.packs) {
      ...
      expect(pack.files.map((f) => f.number)).toEqual(
        Array.from({ length: 90 }, (_, i) => i + 1),
      );
      let total = 0;
      for (const file of pack.files) {
        const path = pack.bundled
          ? `../assets/voice/aria/${file.number}.wav`
          : `../voice-packs/public/packs/${pack.id}/${pack.revision}/${file.number}.wav`;
        ...
        expect(numberCallDelayMs(file.number, 3)).toBeGreaterThanOrEqual(
          file.durationMs + 250,
        );
      }
    ...
    const bank = readFileSync(
      new URL("../src/voiceBank.ts", import.meta.url),
      "utf8",
    );
    expect((bank.match(/require\(/g) ?? []).length).toBe(90);
    expect(bank).not.toContain("en-female");
    expect(bank).not.toContain("voice-packs/public");
  }, 30_000);
});
