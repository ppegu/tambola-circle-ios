...
assert.equal(
  groups.reduce((n, g) => n + g.screens.length, 0),
  36,
);
...
for (const panel of fixture.panels) {
  ...
  for (let col = 0; col < 9; col++) {
    ...
    for (const value of values)
      assert.ok(Number.isInteger(value) && value >= low && value <= high);
    assert.deepEqual(
      values,
      [...values].sort((a, b) => a - b),
    );
...
for (const phrase of [
  "readyWindowSeconds | 120",
  "reviewWindowSeconds | 120",
  "approved_by_timeout",
  "proof_accepted_by_timeout",
  "authorityEpoch",
  "without a winner",
])
  ...
  JSON.stringify(
    {
      ...
      fixture:
        "6 valid panels, 90 unique numbers; Half contains 3 panels / 45 unique numbers",
      gallery:
        "72 navigation targets covering 36 unique screens; local links valid; JavaScript syntax valid",
      scope:
        "Static design-artifact verification only; browser/native/game runtime not tested",
    },
    null,
    2,
  ),
