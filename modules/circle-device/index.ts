...
export type VoiceFileDownload = VoiceFile & { id: string; revision: string; url: string; token: string };
type CircleDevice = {
  ...
  getVoicePackFiles?(id: string, revision: string, files: VoiceFile[]): Promise<string[] | null>;
...
function requireDevice(): CircleDevice {
  ...
  if (!native) throw new Error('CircleDevice is missing. Rebuild the native app after installing dependencies.');
  return native;
}
