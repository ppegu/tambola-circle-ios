...
RCT_EXPORT_METHOD(requestMicrophonePermission:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject) {
  AVAudioSessionRecordPermission status = AVAudioSession.sharedInstance.recordPermission;
  if (status == AVAudioSessionRecordPermissionGranted) { resolve(@YES); return; }
  if (status == AVAudioSessionRecordPermissionDenied) { resolve(@NO); return; }
