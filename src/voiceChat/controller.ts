  ...
  toggleMic() {
    ...
    void this.enqueue(async () => {
      ...
      if (!this.wantedMic || revision !== this.micRevision || this.ended)
        return;
      const peer = this.ports.peer(this.config, false, this.failed);
      this.publish = peer;
      try {
        ...
        await peer.applyAnswer(reply.sessionDescription);
        ...
        if (!this.currentMic(peer, revision)) this.closePeer(peer);
      }
    });
  }
