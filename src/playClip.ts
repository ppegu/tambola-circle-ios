...
export function createClipPlayer(
  preparation?: ReturnType<typeof createVoicePreparation>,
  revisions: Record<number, string> = {},
): StartClip {
  return (clip, ended, failed) => {
    let id = String(++nextClip);
    const uri = voiceAssetUri(
      clip,
      typeof clip === "number" ? revisions[clip] : undefined,
    );
    let active = true;
    const play = async () => {
      try {
