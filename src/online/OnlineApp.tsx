...
export function OnlineApp({
  ...
  onStop,
  onGameplay,
  initialMode, sound, onSound, audioError,
}: {
  model: OnlineModel;
  sound?: boolean; onSound?: (enabled: boolean) => void; audioError?: string;
  onHome: () => void;
  onOffline: () => void;
  ...
  useEffect(() => {
    void setGameUi(immersive, immersive).catch(() => {});
    return () => {
      onGameplay(false);
      void setGameUi(false, false).catch(() => {});
    };
  }, [immersive, onGameplay]);
  return (
    <GameBackground
      style={[
        ui.page,
    ...
    >
      <StatusBar hidden={immersive} barStyle="light-content" />
      {!!audioError && model.snapshot?.phase === 'live' && <View style={ui.error}><Text style={{ color: color.red, fontSize: 12 }}>{tr("Audio interrupted. Tap the current number to retry.")}</Text></View>}
      {!model.loaded ? (
