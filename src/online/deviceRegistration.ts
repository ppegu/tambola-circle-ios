...
let recording: Promise<{ identity: OnlineIdentity; access?: SignedAccess }> | undefined;
...
export function recordDeviceAccess(): Promise<{ identity: OnlineIdentity; access?: SignedAccess }> {
  ...
  recording = (async () => {
    ...
    const payload = { deviceUuid: identity.deviceUuid, info, ...(info.platform === 'android' ? { accessCapability: 1 } : {}) };
    ...
    const response = await request<{ access?: SignedAccess }>("/v2/devices/open", {
      method: "POST",
      token: identity.key,
      body: payload,
    });
