...
export function useRemoteList<T>(
  ...
  const pending = useRef<{ identity: string; promise: Promise<void> } | null>(
    null,
  );
  useEffect(() => {
  ...
  useEffect(() => {
    setState({ items: [], loaded: false, loading: false, error: "" });
  }, [identity]);
  const refresh = useCallback((): Promise<void> => {
    ...
    const promise = request<Record<string, T[]>>(path, { token })
      .then((data) => {
        if (!Array.isArray(data[field]))
          throw new Error("Could not load this list. Please try again.");
        if (active())
          setState({
            items: data[field]!,
            loaded: true,
