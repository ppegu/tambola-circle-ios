...
export async function chooseAvatarPhoto(): Promise<string | null> {
  const native = NativeModules.CirclePhotos as { pick?: () => Promise<string | null> } | undefined;
...
export async function uploadAvatarPhoto(base64: string): Promise<string> {
  ...
  try {
    const response = await fetch(API_URL + '/v2/avatars', { method: 'POST', signal: controller.signal, headers: {
      ...accessHeaders(), Authorization: `Bearer ${identity.key}`, 'Content-Type': 'image/jpeg',
    }, body: bytes });
    const data = await response.json();
    if (data.access) await dispatchAccess(data.access);
    if (!response.ok) throw new ApiError(data.error ?? t('Could not save your photo. Please try again.'), response.status);
    if (!isAvatarPhoto(data.photo)) throw new Error(t('Could not save your photo. Please try again.'));
    return data.photo;
