...
export function OnlineEntry({
  ...
  model: OnlineModel;
  invite: { tableId: string; invite: string } | null;
  onInviteUsed: () => void;
  onOffline: () => void;
  onHome?: () => void;
  ...
  const [mode, setMode] = useState<
    "home" | "profile" | "coins" | "create" | "join"
  >(initialMode);
  ...
  }, [person?.id, invite?.tableId, invite?.invite]);
  const title = !person
    ? tr("Registration")
    : {
        home: tr("Play online"),
        profile: tr("Your profile"),
        coins: tr("Coin shop"),
        create: tr("Create table"),
        join: tr("Join table"),
      }[mode];
  const back = () => {
    if (mode === initialMode || mode === "home") onHome?.();
    else setMode("home");
  };
  useEffect(() => {
    if (
      ...
      !previewRegistration &&
      !policy &&
      (mode === "home" || mode === "coins")
    )
      return;
    const sub = BackHandler.addEventListener("hardwareBackPress", () => {
      if (policy) setPolicy(false);
      else if (previewRegistration) setPreviewRegistration(false);
      else back();
      return true;
    });
    return () => sub.remove();
  ...
  if (!person || previewRegistration)
    return (
      <RegistrationScreen
        busy={model.busy}
        phoneEntryMode={phoneEntryMode}
        onGuest={onOffline}
        onBack={() => {
          if (previewRegistration) setPreviewRegistration(false);
          else onHome?.();
        }}
        onRegister={(name, mobile, avatarId, avatarPhoto) => {
          void model
            .register(name, mobile, avatarId, avatarPhoto)
            .then((ok) => {
              if (ok) setPreviewRegistration(false);
            });
        }}
      />
    );
    ...
    return (
      <OnlineHub
        ...
        invitation={
          invite ? (
            <GameCard>
              <Text style={ui.heading}>{tr("Your invitation is ready!")}</Text>
              <Action
                disabled={model.busy}
                onPress={() => {
                  setJoinInvite(invite);
  ...
  if (person && mode === "coins")
    return <WalletScreen model={model} onBack={back} />;
  return (
    <View style={ui.page}>
      <PageHeader
        title={title}
        ...
        <>
          ...
          {mode === "profile" && (
            <GameCard>
              ...
              <Action small secondary onPress={() => setPolicy(true)}>
                {tr("Terms & Privacy Policy")}
              </Action>
              <Action onPress={() => setMode("home")}>
                {tr("Back to the fun")}
              </Action>
            </GameCard>
          )}
      ...
      {avatarsOpen && (
        <AvatarPicker
          ...
          onSelect={async (id, photo) => {
            if (await model.setAvatar(id, photo)) setAvatarsOpen(false);
          }}
        />
      )}
    </View>
