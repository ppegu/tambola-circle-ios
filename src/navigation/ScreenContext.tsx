...
export type Route = 'Invitations' | 'InvitePlayers' | 'Home' | 'Caller' | 'Online' | 'Registration' | 'Profile' | 'CreateTable' | 'JoinTable' | 'Table' | 'Tickets' | 'TableSettings' | 'MemberTickets' | 'Numbers' | 'Players' | 'Wallet' | 'Transactions' | 'TopUp' | 'PastGames' | 'PastGame' | 'Preferences' | 'Voices' | 'Policy';
...
export type ScreenNavigation = {
  ...
  id: string; name: Route; active: boolean;
...
export function ScreenSafeArea({ children }: { children: React.ReactNode }) {
  ...
  return navigation ? <>{children}</> : <SafeAreaProvider>{children}</SafeAreaProvider>;
}

/** Full screens use the native stack; compact confirmations remain native modals. */
export function ScreenSurface({ children, onClose, onShow }: { children: React.ReactNode; onClose: () => void; onShow?: () => void }) {
  const navigation = useScreenNavigation();
  const appeared = useScreenAppeared();
  useEffect(() => { if (navigation && appeared) onShow?.(); }, [!!navigation, appeared, onShow]);
  return navigation ? <>{children}</> : <Modal visible hardwareAccelerated animationType="slide" statusBarTranslucent navigationBarTranslucent onRequestClose={onClose} onShow={onShow}>{children}</Modal>;
}
