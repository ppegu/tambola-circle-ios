...
export function ApplicationServices() {
  ...
  useEffect(() => {
    ...
    const openInvitationsFromPush = (message: {
    ...
    const setupMessaging = async () => {
      ...
      try {
        ...
        unsubscribe.push(
          ...
          client.onNotificationOpenedApp((message) =>
            openInvitationsFromPush(message.data),
          ),
        );
        configured = true;
        try {
        ...
        if (active) openInvitationsFromPush(initial?.data);
      } catch (error) {
        if (active)
