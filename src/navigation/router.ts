...
export function trackScreen(id: string, name: Route) {
  mounted.set(id, name);
  return () => {
    mounted.delete(id);
...
export function pushScreen(
  name: Route,
  params: RouteParams = {},
  from = navigationStore.getState().id,
) {
  void transition(() =>
    Navigation.push(from, {
      component: {
        id: `circle-${name}-${++sequence}`,
        name: `Circle.${name}`,
        passProps: params,
      },
    }),
              ...
              component: {
                ...
                options: {
                  ...
                  animations: {
                    showModal: {
                      waitForRender: true,
                      translationY: { from: height, to: 0, duration: 240 },
                    },
                    dismissModal: {
                      translationY: { from: 0, to: height, duration: 200 },
                    },
                  },
                },
              },
...
export function homeScreen() {
  void transition(() => Navigation.popToRoot(navigationStore.getState().id));
}
export function returnToOnline() {
  ...
  void transition(async () => {
    ...
    return Navigation.push("circle-home", {
      component: { id: `circle-Online-${++sequence}`, name: "Circle.Online" },
    });
  });
}
export function startNavigation() {
      ...
      ? {
          push: {
            ...
            content: {
              ...
              exit: {
                translationX: { from: 0, to: -width * 0.22, duration: 260 },
              },
            },
          },
          pop: {
            content: {
              enter: {
                translationX: { from: -width * 0.22, to: 0, duration: 240 },
              },
  ...
  Navigation.setDefaultOptions({
    ...
    layout: {
      ...
      componentBackgroundColor: "#290435",
  ...
  return Navigation.setRoot({
    root: {
      stack: {
        id: "circle-stack",
        children: [{ component: { id: "circle-home", name: "Circle.Home" } }],
      },
    },
  });
}
