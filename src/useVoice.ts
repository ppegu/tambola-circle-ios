...
function bankFor(id: string, preview = false) {
  ...
  if (preview) {
    const sources = CALLER_VOICES.map((voice) => {
      ...
      return {
        uri: voiceAssetUri(VOICE_PREVIEW_ASSETS[voice.id]!, sample.sha256),
        durationMs: sample.durationMs,
      };
    });
    const index = CALLER_VOICES.findIndex((voice) => voice.id === pack.id);
    return {
      key: "bundled-previews",
      sources,
      calls: { 47: { full: sources[index]!.uri } },
    };
  }
  const files = pack.files;
  const sources = files.map((file) => ({
    uri: pack.bundled
      ? voiceAssetUri(VOICE_BANK.female[file.number]!.full, file.sha256)
      : (voiceFileUri(pack.id, file.number) ??
        voiceDownloadUrl(pack.id, file.number)),
    durationMs: file.durationMs,
  }));
  return {
    key: `${pack.id}:${pack.revision}:${files.length}`,
    sources,
    calls: Object.fromEntries(
      files.map((file, index) => [file.number, { full: sources[index]!.uri }]),
    ),
  };
}
...
export function useVoice(
  ...
  const [preparing, setPreparing] = useState(
    enabled && !!CircleDevice.prepareClips,
  );
  const { selected, loading: restoringVoice } = useCallerVoices();
  const bank = useRef<{ key: string; sources: VoiceClipSource[] }>({
    key: "",
    sources: [],
  });
  const [preparation] = useState(() =>
    createVoicePreparation(CircleDevice, () => bank.current.sources),
  );
  const warmGeneration = useRef(0),
    generation = useRef(0);
  const voiceVolume = useGamePreferences((state) => state.voiceVolume);
  const inGame = useRef(gameActive);
  inGame.current = gameActive;
  ...
  const [player] = useState(() =>
    createAnnouncementPlayer(
      ...
      () => {
        ...
        pending?.reject(
          new Error(
            "Preview unavailable. Check your connection and try again.",
          ),
        );
  ...
  const chooseBank = useCallback(
    (next: ReturnType<typeof bankFor>) => {
      if (bank.current.key !== next.key) {
        bank.current = next;
  ...
  warm.current = () => {
    ...
    if (
      previewMounted.current ||
      player.isSpeaking() ||
      AppState.currentState !== "active" ||
      !enabled ||
      !CircleDevice.prepareClips
    )
      return;
    const ticket = ++warmGeneration.current;
    chooseBank(bankFor(voicePacks.getSnapshot().selected));
    setPreparing(true);
    void preparation.prepare().then(
      () => {
        if (ticket === warmGeneration.current) {
          setPreparing(false);
          setError("");
        }
      },
      () => {
        if (ticket === warmGeneration.current) {
          setPreparing(false);
          setError("Could not prepare voice. Tap the speaker to retry.");
        }
      },
    );
  };
  useEffect(() => {
    void CircleDevice.setVoiceVolume?.(voiceVolume).catch(() => {});
  }, [voiceVolume]);
  ...
  useEffect(() => {
    const update = () => {
      ...
      void setGameAudioActive(gameActive && active).catch(() =>
        setError("Audio unavailable. Visual calling still works."),
      );
      if (!active) {
        warmGeneration.current++;
        setPreparing(false);
        void preparation.release().catch(() => {});
      } else warm.current();
    };
    update();
    ...
    return () => {
      ...
      warmGeneration.current++;
      stop();
      void setGameAudioActive(false).catch(() => {});
    };
  ...
  useEffect(() => {
    stop();
  }, [enabled, voice, callPause, stop]);
  const call = useCallback(
    (
      n: number,
      force = false,
      id = voicePacks.getSnapshot().selected,
      audition = false,
    ) => {
      if (
        (!enabled && !force) ||
      ...
      const next = bankFor(id, audition),
        clip = next.calls[n];
      ...
      void setGameAudioActive(true).catch(() => {
        if (current === generation.current)
          setError("Audio unavailable. Tap the speaker to retry.");
      });
      player.speak(clip, 0);
      return true;
    },
  ...
  useEffect(
    () =>
      registerVoicePreview({
        play: (id) =>
          new Promise<void>((resolve, reject) => {
            stop();
            preview.current = { resolve, reject };
            if (!call(47, true, id, true)) {
              preview.current = undefined;
              reject(
                new Error(
                  "Preview is unavailable while the app is in the background.",
                ),
              );
            }
          }),
        ...
        prepare: async () => {
          ...
          if (
            ticket !== previewGeneration.current ||
            AppState.currentState !== "active"
          )
            return;
          stop();
          chooseBank(bankFor("aria", true));
        ...
        release: () => {
          ...
          void previewCleanup.current.then(() => {
            if (!previewMounted.current) warm.current();
          });
        },
      }),
    [call, stop],
  ...
  return {
    ...
    preparing: preparing || (enabled && restoringVoice),
    error,
  };
}
