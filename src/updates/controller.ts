...
import {
  collectDeviceInfo,
  recordDeviceAccess,
} from "../online/deviceRegistration";
...
const CACHE = "tambola.circle.access.v1",
  DISMISSED = "tambola.circle.update.dismissed",
  PENDING = "tambola.circle.update.install";
...
export function isAccessCheckPending(state: UpdateState, hasRelease: boolean) {
  ...
  return (
    ...
    (state.checking &&
      !state.success &&
      (!state.permitted || (state.details && !hasRelease)))
  );
}
export class UpdateController {
  ...
  async start() {
    ...
    try {
      ...
      this.change({
        ...
        dismissed: await AsyncStorage.getItem(DISMISSED),
      });
      ...
      if (raw) {
        try {
          ...
          if (
            access &&
            Number.isFinite(cache.receivedWall) &&
            Number.isFinite(cache.receivedElapsed) &&
            Number.isFinite(cache.lastWall) &&
            typeof cache.bootId === "string"
          ) {
            this.cache = cache;
            this.change({ access });
          ...
          /* A corrupt cache cannot authorize access. */
        }
      }
      ...
      this.change({
        ready: true,
        error: "We couldn’t check app access. Please try again.",
      });
    }
  }
  stop() {
  ...
  private async verify(
    ...
    if (!isSignedAccess(value))
      throw new Error("Missing signed access decision");
    const publicKey = (keys as Record<string, string>)[value.keyId];
    if (
      !publicKey ||
      !(await updater().verifyAccess(value.payload, value.signature, publicKey))
    )
      throw new Error("Invalid policy signature");
    const access = parseAccess(JSON.parse(value.payload));
    if (
      !access ||
      access.deviceUuid !== this.identity?.deviceUuid ||
      access.packageId !== this.state.info?.appId ||
      (fresh && access.installedBuild !== Number(this.state.info?.appBuild))
    )
      throw new Error("Access decision does not match this installation");
    return access;
  }
  accept = (value: unknown): Promise<void> => {
    const next = this.serial.then(async () => {
      const access = await this.verify(value, true),
        previous = this.state.access;
      if (
        previous &&
        (access.revision < previous.revision ||
          (access.revision === previous.revision &&
            access.issuedAt <= previous.issuedAt))
      )
        throw new Error("Stale policy response");
      const now = await updater().clock();
      this.cache = {
        signed: value as CachedAccess["signed"],
        receivedWall: now.wall,
        receivedElapsed: now.elapsed,
        bootId: now.bootId,
        lastWall: now.wall,
      };
      this.lastOnlineElapsed = now.elapsed;
      const upgraded =
        !!previous && access.installedBuild > previous.installedBuild;
      const success =
        !denied(access.decision) &&
        (upgraded ||
          (this.targetInstalled > 0 &&
            Number(this.state.info?.appBuild) >= this.targetInstalled));
      ...
      this.change({
        access,
        permitted: !denied(access.decision),
        ready: true,
        error: "",
        ...(success
          ? { success: true, details: false, installAttempted: false }
          : {}),
      });
      if (
        ["device_blocked", "app_locked"].includes(access.decision) ||
        previous?.release?.id !== access.release?.id
      )
        await updater().pause();
      await storage.setItem(CACHE, JSON.stringify(this.cache));
      if (success) {
        await updater().clearCompleted();
        await storage.deleteItem(PENDING);
        this.targetInstalled = 0;
      }
    });
  ...
  async checkLease() {
    const access = this.state.access,
      cache = this.cache;
    if (!access || !cache || denied(access.decision)) {
      this.change({ permitted: false });
      return;
    }
    const now = await updater().clock();
    ...
    const onlineOnly =
      ...
      now.elapsed - this.lastOnlineElapsed < 30_000;
    const permitted =
      access.installedBuild === Number(this.state.info?.appBuild) &&
      (onlineOnly || usableLease(access, cache, now));
    this.change({ permitted });
    if (now.wall > cache.lastWall) {
      cache.lastWall = now.wall;
      const save = this.serial.then(async () => {
        if (this.cache === cache)
          await storage.setItem(CACHE, JSON.stringify(cache));
      });
      this.serial = save.catch(() => undefined);
      await save;
    }
  }
  refresh = (): Promise<void> => {
    ...
    this.pending = (async () => {
      ...
      try {
        ...
        this.change({ checking: false, ready: true });
  ...
  async tick(resumed = false) {
    ...
    await this.checkLease().catch(() => this.change({ permitted: false }));
    const interval =
      this.state.access?.expiresAt === this.state.access?.issuedAt
        ? 25_000
        : 300_000;
    if (
      resumed ||
      (Date.now() - this.lastCheck >= interval &&
        Date.now() - this.lastAttempt >= 30_000)
    )
      await this.refresh();
  }
  pollTransfer = async () => {
    try {
      const transfer = await updater().getState();
      if (!this.stopped) this.change({ transfer });
    } catch {
      /* Retryable through the screen. */
    }
  };
  show = () => {
    this.change({ details: true, success: false, error: "" });
    void this.refresh();
  };
  close = () => {
    if (this.state.permitted) this.change({ details: false, success: false });
  };
  dismiss = async () => {
    const id = this.state.access?.release?.id;
    if (id && this.state.permitted) {
      await AsyncStorage.setItem(DISMISSED, id);
      this.change({ dismissed: id });
    }
  };
      ...
      this.change({
        error:
          error instanceof Error
            ? error.message.replace(/^.*Exception:\s*/, "")
            : "Please try again.",
      });
    } finally {
  ...
  get release(): ReleaseDescriptor | undefined {
    ...
    return release &&
      release.versionCode > Number(this.state.info?.appBuild) &&
      !["device_blocked", "app_locked"].includes(this.state.access!.decision)
      ? release
      : undefined;
  }
  download = () =>
    this.action(async () => {
      ...
      await updater().download(JSON.stringify(this.release), API_URL);
      this.change({ details: true, installAttempted: false });
    });
  ...
  install = () =>
    this.action(async () => {
      ...
      this.change({ installAttempted: true });
    });
}
