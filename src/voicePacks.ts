...
export const voicePack = (id: string) =>
  CALLER_VOICES.find((pack) => pack.id === id) ?? CALLER_VOICES[0]!;
...
export const voicePacks = createVoicePackManager(
  CALLER_VOICES,
  CircleDevice,
  {
    load: () => AsyncStorage.getItem(key),
    save: (id) => AsyncStorage.setItem(key, id),
  },
  VOICE_CDN,
);
void voicePacks.initialize();
export function useCallerVoices() {
  return useSyncExternalStore(voicePacks.subscribe, voicePacks.getSnapshot);
}
export function voiceFileUri(id: string, number: number): string | undefined {
  const pack = voicePack(id),
    index = pack.files.findIndex((file) => file.number === number);
  return voicePacks.getSnapshot().installed[id]?.[index];
}
export function voiceDownloadUrl(id: string, number: number) {
  const pack = voicePack(id);
  return `${VOICE_CDN}/packs/${id}/${pack.revision}/${number}.wav`;
}
