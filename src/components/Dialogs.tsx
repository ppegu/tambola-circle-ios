...
export function GameMenu({
  ...
  useLanguage();
  return <Sheet title={tr("Game menu")} onClose={onClose}><View style={{ gap: 10, paddingBottom: 8 }}><GameButton glyph="refresh" onPress={onRestart}>{tr("Restart new game")}</GameButton><GameButton glyph="home" tone="purple" onPress={onHome}>{tr("Back to home")}</GameButton><GameButton glyph="play" tone="cyan" onPress={onClose}>{tr("Keep playing")}</GameButton></View></Sheet>;
}
...
export function SettingsDialog({ model, onClose, onPreviewVoice }: { model: AppModel; onClose: () => void; onPreviewVoice: (number: number) => void }) {
  useLanguage();
  return <GamePreferencesSheet onClose={onClose} sound={model.local.preferences.sound} onSound={sound => model.updatePreferences({ sound, voice: 'female' })} onPreview={() => onPreviewVoice(23)} onReplay={model.local.history.length ? () => onPreviewVoice(model.local.history.at(-1)!) : undefined} />;
}
      ...
      {newCode ? (
        <>
          <Text style={styles.intro}>
            {tr("Keep this code somewhere safe. It is the only way to reset your password, and is shown only once.")}</Text>
          ...
          >
            ...
            <Text style={styles.help}>{tr("I have saved my recovery code")}</Text>
          </Pressable>
        ...
        <>
          <Text style={styles.intro}>{tr("You’re ready to play as a guest.")}</Text>
          <View style={styles.note}>
            <Text style={styles.noteText}>
              {tr("Accounts are not available in this build. Your game and preferences are saved on this device, and the caller works offline.")}</Text>
          </View>
        ...
        <>
          ...
          <Text style={styles.help}>
            {tr("Your calling preferences sync across your devices. Each device keeps its own game.")}</Text>
            ...
            <>
              ...
              >
                <Text style={[styles.link, { color: "#a82b36" }]}>
                  {tr("Delete account and cloud data")}</Text>
              </Pressable>
            </>
          ) : (
            <>
              <View style={styles.note}>
                <Text style={styles.noteText}>
                  {tr("This permanently deletes your account, saved cloud preferences, and all account sessions. Your local game will also be cleared.")}</Text>
              </View>
              <TextInput
              ...
              >
                {tr("Permanently delete account")}</Button>
              <Pressable
                style={styles.linkButton}
        ...
        <>
          ...
          <Text style={styles.help}>
            {tr("Guest preferences use a random app ID. No hardware ID, ad tracking, or permissions are used.")}</Text>
          <Pressable
            disabled={busy}
            accessibilityRole="button"
            onPress={onClose}
          ...
          {profile?.kind === "guest" && (
            ...
            >
              <Text style={[styles.help, { color: "#a82b36" }]}>
                {tr("Reset guest data")}</Text>
            </Pressable>
          )}
          {deleting && (
            <>
              <Text style={styles.help}>
                {tr("This clears your game and deletes this guest’s saved cloud preferences.")}</Text>
              <Button
                danger
                disabled={busy}
                onPress={() => {
              ...
              >
                {tr("Confirm reset guest data")}</Button>
            </>
          )}
        </>
      )}
