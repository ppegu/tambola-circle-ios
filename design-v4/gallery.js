...
function render() {
  ...
  visible = screens.filter(
    (s) =>
      (mode === "all" || s.status === mode) &&
      (group === "all" || s.group === group) &&
      `${s.title} ${s.group} ${s.note} ${s.revision}`
        .toLowerCase()
        .includes(query),
  );
  ...
  visible.forEach((screen, index) => {
    ...
    button.setAttribute("aria-label", `Inspect ${screen.title}`);
    const img = new Image();
    img.src = screen.path;
  ...
  document.querySelector("#count").textContent =
    `${visible.length} screens · ${screens.length} images in the complete collection`;
  document.querySelector("#empty").hidden = visible.length > 0;
}
function show() {
  const screen = visible[selected];
  if (!screen) return;
  ...
  for (const [id, text] of [
    ...
    ["position", `${selected + 1} / ${visible.length}`],
    ["dimensions", `${screen.width} × ${screen.height} · Original PNG`],
  ])
