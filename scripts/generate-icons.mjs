...
const artwork = 'data:image/png;base64,' + (await readFile(path.join(root, 'assets/game-v3/game-hero.png'))).toString('base64');
const source = (await readFile(path.join(root, 'assets/icon.svg'), 'utf8')).replace('game-v3/game-hero.png', artwork);
...
await writeFile(path.join(root, 'ios/TambolaCircle/Images.xcassets/AppIcon.appiconset/icon.png'), full);
await writeFile(path.join(root, 'ios/TambolaCircle/Images.xcassets/LaunchIcon.imageset/icon.png'), full);
...
for (const [density, size] of [['mdpi',48],['hdpi',72],['xhdpi',96],['xxhdpi',144],['xxxhdpi',192]]) {
  ...
  const splash = path.join(root, 'android/app/src/main/res/drawable-' + density, 'splashscreen_logo.png');
  ...
  for (const name of ['ic_launcher', 'ic_launcher_round', 'ic_launcher_foreground']) {
    ...
    await unlink(path.join(dir, name + '.webp')).catch(error => { if (error.code !== 'ENOENT') throw error; });
    await writeFile(path.join(dir, name + '.png'), render(name.endsWith('foreground') ? foreground : source, name.endsWith('foreground') ? size * 2.25 : size));
  }
}
