...
if (existsSync(privatePath))
  material = JSON.parse(readFileSync(privatePath, "utf8"));
else {
...
const keys = existsSync(keysPath)
  ...
  : {};
if (keys[material.keyId] && keys[material.keyId] !== material.publicKey)
  throw new Error("Key ID conflict; explicit rotation is required.");
...
if (process.argv.includes("--local")) {
  ...
  for (const [key, value] of Object.entries({
    UPDATE_SIGNING_KEY: material.privateKey,
    UPDATE_ADMIN_TOKEN: material.adminToken,
    UPDATE_KEY_ID: material.keyId,
  })) {
    if (!new RegExp("^" + key + "=", "m").test(local))
      local += "\n" + key + "=" + value + "\n";
  }
  writeFileSync(envPath, local, { mode: 0o600 });
}
console.log(
  "Update signing public key prepared. Private material stays in ignored .credentials; no existing key was replaced.",
);
