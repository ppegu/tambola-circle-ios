...
const allowed = new Set([
  ...
  "com.ppegu.tambola.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION",
  "android.permission.RECORD_AUDIO",
]);
for (const [, permission] of permissions.matchAll(
...
assert(
  info.includes(`application-label:'${config.displayName}'`),
  "Incorrect launcher label",
);
assert(
  !/^application-label[^:]*:.*(?:legacy|internal)/im.test(info),
  "Unexpected internal branding in translated app labels",
);
assert(
  info.includes(
    `versionCode='${config.android.versionCode}' versionName='${config.version}'`,
  ),
...
const voiceManifest = JSON.parse(
  readFileSync("shared/voicePacks.json", "utf8"),
).packs.flatMap((pack) =>
  pack.bundled ? pack.files : pack.files.filter((clip) => clip.number === 47),
);
assert.equal(
  voiceEntries.length,
  voiceManifest.length,
  "All 90 female number calls and four offline previews must be bundled",
);
const bundledHashes = voiceEntries
  .map((entry) =>
    createHash("sha256")
      .update(execFileSync("unzip", ["-p", apkPath, entry]))
      .digest("hex"),
  )
  .sort();
assert.deepEqual(
  bundledHashes,
  voiceManifest.map((clip) => clip.sha256).sort(),
  "Bundled voice bytes differ from the checked voice packs",
);
const nativeBridgePresent = entries
...
assert(
  nativeBridgePresent,
  "Native SIM picker / game UI bridge missing from APK",
);
const voiceStoragePresent = entries
  .filter((entry) => /^classes\d*\.dex$/.test(entry))
  .some((entry) =>
    execFileSync("unzip", ["-p", apkPath, entry], {
      maxBuffer: 64 * 1024 * 1024,
...
assert(
  voiceStoragePresent,
  "Native verified offline voice storage missing from APK",
);
const updaterPresent = entries
  .filter((entry) => /^classes\d*\.dex$/.test(entry))
  .some((entry) =>
    execFileSync("unzip", ["-p", apkPath, entry], {
      maxBuffer: 64 * 1024 * 1024,
...
assert(
  !cert.includes("CN=Android Debug"),
  "Release must use the app signing key",
);
assert(
  cert.includes(
    "2d07b75319bacbafaa3750671d7b07e4beee8e4421952921fd79ec20c6bd0071",
  ),
  "Release certificate must match existing installations",
);
console.log(
  "Verified release manifest, standalone JS bundle, native CircleDevice bridge, 90 Aria calls, four offline previews, and the existing release signature.",
);
