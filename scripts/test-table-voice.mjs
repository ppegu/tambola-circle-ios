...
const host = await player("Voice QA Host", "+15550005001"),
  guest = await player("Voice QA Guest", "+15550005002");
let s = (
  await post(
    "/v2/tables",
    host.key,
    { createId: randomUUID(), name: "Voice QA", visibility: "private" },
    201,
  )
).snapshot;
const path = `/v2/tables/${s.id}/voice`,
  join = { op: "join", clientId: randomUUID() };
...
s = (
  await post(`/v2/tables/${s.id}/commands`, host.key, {
    id: randomUUID(),
    roundId: s.roundId,
    authorityEpoch: s.authorityEpoch,
    type: "KICK",
    payload: { memberId: guest.id },
  })
).snapshot;
await post(path, guest.key, join, 403);
console.log(
  `PASS: ${checks} table voice API checks (auth, membership, removed member, request validation, bounded body, unavailable configuration).`,
);
