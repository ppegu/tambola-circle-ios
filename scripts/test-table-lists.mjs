...
const host = await user("List QA Host", "001"),
  guest = await user("List QA Guest", "002"),
  outsider = await user("List QA Visitor", "003");
...
async function command(player, type, payload = {}) {
  s = (
    await api(`/v2/tables/${tableId}/commands`, player.key, {
      type,
      payload,
      id: randomUUID(),
      roundId: s.roundId,
...
const tables = await eventually(
  () => api("/v2/tables", host.key),
  (x) => x.tables[0]?.playerCount === 2,
);
assert.deepEqual(
  tables.tables[0].players.map((p) => p.name),
  [host.name, guest.name],
);
assert.ok(!JSON.stringify(tables).includes("mobile"));
assert.ok(!JSON.stringify(tables).includes(s.invite));
...
await command(guest, "MARK", {
  panel: 0,
  cell: s.members[guest.id].panels[0].flat().findIndex((n) => n !== null),
  marked: true,
  version: 0,
});
await command(host, "END");
const history = await eventually(
  () => api("/v2/table-history", host.key),
  (x) => x.games.length === 1,
);
assert.equal(history.games[0].round, 1);
assert.equal(history.games[0].playerCount, 2);
assert.equal(history.games[0].completed, false);
const detail = await api(`/v2/table-history/${tableId}/1`, guest.key);
assert.equal(detail.game.name, "List verification");
assert.equal(detail.tickets.length, 2);
assert.equal(detail.tickets[0].panels.length, 3);
assert.ok(
  Object.values(detail.tickets.find((p) => p.id === guest.id).marks).some(
    Boolean,
  ),
);
for (const forbidden of ["mobile", "invite", "deviceKey", "code"])
  assert.ok(!Object.hasOwn(detail, forbidden));
assert.ok(!JSON.stringify(detail).includes(host.mobile));
assert.ok(!JSON.stringify(detail).includes(s.invite));
...
await api(`/v2/table-history/${tableId}/1`, outsider.key, undefined, 404);
await command(host, "NEXT_ROUND");
assert.equal(s.round, 2);
await eventually(
  () => api("/v2/tables", host.key),
  (x) => x.tables[0]?.round === 2 && x.tables[0]?.phase === "lobby",
);
...
await eventually(
  () => api("/v2/table-history", guest.key),
  (x) => !x.games.length,
);
await api(`/v2/table-history/${tableId}/1`, guest.key, undefined, 404);
await command(host, "END");
console.log(
  `PASS: ${checks} API checks. Empty lists, membership counts, saved tickets/marks, past rounds after restart, outsider/late-join/kicked access, and private-data exclusion.`,
);
