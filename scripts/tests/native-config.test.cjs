...
test("Metro ignores transient build folders with both crawler and Windows watcher paths", () => {
  ...
  for (const folder of [
    ...
    ".tools",
    "server/.wrangler",
    "server/legacy-proxy/.wrangler",
    "android/app/build",
  ]) {
    for (const value of [
      folder + "/tmp/bundle",
      path.join(root, folder, "tmp/bundle"),
    ]) {
...
test("only public app configuration is exposed, with process > local > base precedence", () => {
  ...
  try {
    fs.writeFileSync(
      path.join(dir, ".env"),
      "APP_PUBLIC_API_URL=https://base.example\nAUTH_SECRET=never-bundle-me\nAPP_PUBLIC_SHARE_URL=https://share.example\n",
    );
    fs.writeFileSync(
      path.join(dir, ".env.local"),
      "APP_PUBLIC_API_URL=https://local.example\n",
    );
    assert.equal(
      publicConfig(dir, {}).__CIRCLE_PUBLIC_API_URL__,
      "https://local.example",
    );
    assert.deepEqual(
      publicConfig(dir, {
        APP_PUBLIC_API_URL: "https://ci.example",
        AUTH_SECRET: "private",
      }),
      {
        __CIRCLE_PUBLIC_API_URL__: "https://ci.example",
        __CIRCLE_PUBLIC_SHARE_URL__: "https://share.example",
      },
    );
    assert.equal(
      publicConfig(dir, { APP_PUBLIC_API_URL: "" }).__CIRCLE_PUBLIC_API_URL__,
      "",
    );
  } finally {
...
test("Babel emits configured native URLs without public placeholders or private environment values", () => {
  ...
  const result = spawnSync(
    ...
    [
      "-e",
      'process.stdout.write(require("@babel/core").transformFileSync("src/config.ts").code)',
    ],
    {
      ...
      env: {
        ...process.env,
        APP_PUBLIC_API_URL: "https://native.example",
        APP_PUBLIC_SHARE_URL: "",
        AUTH_SECRET: "do-not-embed",
      },
    },
  );
