...
for (const [name, h, radius, step] of [
  ["ticket-paper", 140, 10, 9],
  ["paper", 150, 5, 12],
  ["paper-large", 450, 18, 30],
]) {
  const w = 340;
  const holes = Array.from(
    { length: Math.floor((h - 16) / step) },
    (_, i) =>
      `<circle cx="0" cy="${10 + i * step}" r="2" fill="black"/><circle cx="${w}" cy="${10 + i * step}" r="2" fill="black"/>`,
  ).join("");
