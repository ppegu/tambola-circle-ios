...
function TambolaApp() {
  ...
  useEffect(
    () => (Platform.OS === "ios" ? startDeviceRecording() : undefined),
    [],
  );
  ...
  const [onlineStart, setOnlineStart] = useState<"home" | "profile" | "coins">(
    "home",
  );
        ...
        <>
          ...
          {dialog === "settings" && (
            <GamePreferencesSheet
              ...
              sound={model.local.preferences.sound}
              onSound={(sound) => updatePreferences({ sound })}
              onPreview={() => speak(23, true)}
              onReplay={
                model.local.history.length
                  ? () => speak(model.local.history.at(-1)!, true)
                  : undefined
              }
            />
          )}
